...
export default function ClubDetail() {
    ...
    return (
        <View className="flex-1 bg-white">
            <Image source={{ uri: club.image }} className="w-full h-64" resizeMode="cover" />
            ...
            <ScrollView className="flex-1 -mt-6 bg-white rounded-t-3xl px-6 pt-8">
                ...
                <View className="flex-row items-center mb-8 space-x-4">
                    <View className="flex-row items-center bg-gray-100 px-3 py-1 rounded-full">
                        ...
                        <Text className="text-gray-600 text-sm ml-1 font-medium">{club.members} Members</Text>
                    </View>
                    <View className="flex-row items-center bg-gray-100 px-3 py-1 rounded-full ml-2">
                        <Calendar size={14} color="#4B5563" />
                        <Text className="text-gray-600 text-sm ml-1 font-medium">Est. {club.established}</Text>
                    </View>
                </View>

                <Text className="text-lg font-bold text-gray-900 mb-3">About Us</Text>
                <Text className="text-gray-600 leading-7 mb-8 text-base">{club.description}</Text>
            ...
            <View className="absolute bottom-0 w-full p-6 bg-white border-t border-gray-100 shadow-lg">
                {club.type === 'PAID' && (
                    <View className="flex-row justify-between items-center mb-4">
                        ...
                        <Text className="text-xl font-bold text-primary">{club.membershipFee.toLocaleString()} VND</Text>
                    </View>
                )}
                <TouchableOpacity
                    className="w-full bg-primary py-4 rounded-xl items-center shadow-md shadow-indigo-200"
                    onPress={() => router.push({ pathname: '/(student)/clubs/apply', params: { clubId: club.id, clubName: club.name, type: club.type, fee: club.membershipFee } })}
                >
                    <Text className="text-white font-bold text-lg">
                        {club.type === 'FREE' ? 'Apply Now (Free)' : 'Apply for Interview'}
                    </Text>
                </TouchableOpacity>
            </View>
        </View>
